...
bot.hears('Додати клієнта 📝', (ctx) => {
  ctx.reply('Напиши номер картки (наприклад, #0016):');
  currentStep = 'card_number'; 
});

bot.hears('Продовжити абонемент 🔄', (ctx) => {
  ctx.reply('Напиши номер картки для продовження абонементу:');
...
bot.on('text', async (ctx) => {
  ...
  if (currentStep === 'card_number') {
    ...
    if (rowIndex === -1) {
      ...
      ctx.reply('✅ Карта знайдена! 🎉 Напиши колір картки (наприклад, синя картка або червона картка):');
      currentStep = 'color';
    }
  } else if (currentStep === 'color') {
    userData.color = text;
    ctx.reply('📅  дату покупки (наприклад, 12.09.2024):');
    currentStep = 'purchase_date';
  } else if (currentStep === 'purchase_date') {
    userData.purchaseDate = text;
    ctx.reply('💵 Напиши вартість:');
    currentStep = 'price';
  } else if (currentStep === 'price') {
    userData.price = text;
    ctx.reply('✍️ Напиши ПІБ:');
    currentStep = 'fio';
  } else if (currentStep === 'fio') {
    userData.fio = text;
    ctx.reply('📞 Напиши номер телефону:');
    currentStep = 'phone';
  } else if (currentStep === 'phone') {
    userData.phone = text;
    ctx.reply('🎂 Напиши дату народження (наприклад, 23.10.2007):');  
    currentStep = 'dob'; 
  } else if (currentStep === 'dob') { 
    userData.dob = text;
    ctx.reply('🗓 Напиши кількість заходів (наприклад, 4):');
    currentStep = 'events';
  } else if (currentStep === 'events') {
    userData.events = text;
    ctx.reply('🚶 Напиши кількість відвідувань (наприклад, 1):');
    currentStep = 'visits';
  ...
  if (currentStep === 'increment_visits') {
    ...
    if (cardRow !== -1) {
      ...
      ctx.reply('📅 Напиши новую дату покупки:');
